...
class YuanjingEmbed(OpenAIEmbed):
    _FACTORY_NAME = "yuanjing"

    def __init__(self, key, model_name, base_url="https://maas-api.ai-yuanjing.com/openapi/compatible-mode/v1"):
        if not base_url:
            base_url = "https://maas-api.ai-yuanjing.com/openapi/compatible-mode/v1"
        # print(f"Using YuanjingEmbed=== {key} {base_url} ======")
        self.client = OpenAI(api_key=key, base_url=base_url)
        self.model_name = model_name

    def encode(self, texts: list):
        # print(f"YuanjingEmbed Embedding: {len(texts)} ========== {texts}")
        # OpenAI requires batch size <=16
        batch_size = 16
        texts = [truncate(t, 8191) for t in texts]
        ress = []
        ...
        for i in range(0, len(texts), batch_size):
            res = self.client.embeddings.create(
                input=texts[i : i + batch_size], 
                model=self.model_name,
                encoding_format="float"  # Add encoding_format parameter
            )
            try:
                ress.extend([d.embedding for d in res.data])
                total_tokens += self.total_token_count(res)
